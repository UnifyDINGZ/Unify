...
import { AlertCircle, Tag, UserPlus, Wallet, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {cards.map((card) => {
                ...
                return (
                    <div key={card.title} className={`bg-white rounded-2xl border ${card.borderAccent} shadow-sm p-5 flex flex-col gap-4 hover:shadow-md transition-shadow duration-200`}>
                        ...
                        <div className="flex flex-col gap-1">
                            <span className={`${card.isAmount ? "text-2xl" : "text-3xl"} font-bold tracking-tight ${card.countColor} leading-none`}>
                                {card.count}
                            </span>
                            <p className="text-sm font-bold text-gray-800 leading-snug">{card.title}</p>
        ...
        <div className="flex flex-col gap-6">
            <div className="flex justify-between items-end">
                <div className="flex flex-col gap-2">
                    <h1 className="text-2xl text-gray-900 font-semibold font-sans">Good morning, {user.name}</h1>
                    <p className="text-sm text-gray-700">Here are your recommended actions for today.</p>
                </div>

                <div className="relative" ref={calendarRef}>
                    <button onClick={() => setShowCalendar((v) => !v)} className="text-sm text-gray-700 hover:text-gray-900 hover:underline transition-colors cursor-pointer">
